...
const Header = () => {
  ...
  return (
    <div className="bg-[#2B2B2B] font-global z-999 fixed w-full">
      <div className="max-w-[1280px] bg-[#2B2B2B] py-[30px] md:py-[20px] mx-auto px-[30px] xl:px-0 flex items-center justify-between">
        <a href="#">
        ...
        <nav className='hidden md:flex items-center md:gap-[20px] xl:gap-[40px] text-white font-[600]'>
          ...
          <Button name='Sign Up' icon={FaRegUser} />
        </nav>
        <div className="text-white text-[26px] block md:hidden" onClick={handleClick}>
            {nav ? <IoClose /> : <CgMenuLeft />}
        </div>
        <nav className={`absolute flex md:hidden flex-col w-full -z-1 left-0 bg-[#2B2B2B] px-[35px] text-white font-[500] py-[40px] transition-all duration-500 ${nav ? 'top-[70px]' : 'top-[-250px]'}`}>
